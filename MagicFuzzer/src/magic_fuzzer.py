...
class MagicFuzzer:
    ...
    def run_input(self, input):
        ...
        for location in locations:
            ...
            if (
                nombreFuncion not in self.lines_covered
                or numeroDeLinea not in self.lines_covered[nombreFuncion]
            ):
    ...
    def get_covered_locations(self) -> Set[Location]:
        ...
        locations = self.covered_locations.difference(
            [("run_function", 410), ("trace", 378), ("coverage", 382)]
        )
        return (
            locations
            if not self.method_name
                ...
                (f_name, f_line)
                for f_name, f_line in locations
                if f_name == self.method_name
            )
        )
    ...
    def mutate(self, s: str) -> str:
        ...
        return random.choice(
            [insert_random_character, delete_random_character, change_random_character]
        )(s)
    ...
    def run_until_covered(self, n=None) -> int:
        ...
        """
        iteraciones = 0
        while len(self.get_covered_locations()) != self.source_lines - 1:
            iteraciones += 1
            self.fuzz()
        return iteraciones
    ...
    def run_iterations(self, n=None) -> int:
        ...
        """
        if not n:
            return self.run_until_covered()
        else:
            iteraciones = 0
            while iteraciones < n and len(self.get_covered_locations()) != self.source_lines - 1:
                iteraciones += 1
                self.fuzz()
            return iteraciones
